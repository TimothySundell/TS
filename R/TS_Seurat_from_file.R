...
TS_Seurat_from_file <- function(
  ...
  seurat_object[["percent.ribo"]] <- Seurat::PercentageFeatureSet(
    seurat_object,
    pattern = "^RP[SL]"
  )
  message("Calculating proportion of MALAT1 transcripts")
  seurat_object[["percent.malat1"]] <- Seurat::PercentageFeatureSet(
    seurat_object,
    pattern = "MALAT1"
  )
  ...
  if (remove_ig_genes) {
    ...
    seurat_object2 <- seurat_object[
      !grepl("^IG[HKL][VJCADEGM]", rownames(seurat_object)),
    ]
  ...
  seurat_object2 <- Seurat::NormalizeData(
    seurat_object2,
    normalization.method = normalization.method,
    scale.factor = scale.factor,
    verbose = F
  )
  seurat_object2 <- Seurat::FindVariableFeatures(
    seurat_object2,
    selection.method = selection.method,
    nfeatures = nfeatures,
    verbose = F
  )
  seurat_object2 <- Seurat::ScaleData(
    seurat_object2,
    features = rownames(seurat_object2),
    verbose = F
  )
  # Run PCA
  message("Running PCA")
  seurat_object2 <- Seurat::RunPCA(
    seurat_object2,
    features = Seurat::VariableFeatures(object = seurat_object2),
    verbose = F
  )
...
enter which specific PCs you want to use, separated ONLY by commas: "
    ...
    if (grepl(pattern = ",", x = regular_clustering_dims)) {
      ...
      seurat_object2 <- Seurat::FindNeighbors(
        seurat_object2,
        dims = regular_clustering_dims,
        nn.method = nn.method,
        verbose = T
      )
      message("Calculating UMAP coordinates")
      seurat_object2 <- Seurat::RunUMAP(
        seurat_object2,
        dims = regular_clustering_dims,
        verbose = F
      )
      ...
      seurat_object2 <- Seurat::FindNeighbors(
        seurat_object2,
        dims = 1:as.numeric(regular_clustering_dims),
        nn.method = nn.method,
        verbose = T
      )
      message("Calculating UMAP coordinates")
  ...
  seurat_object2 <- Seurat::FindClusters(
    seurat_object2,
  ...
  seurat_object2 <- Seurat::FindClusters(
    seurat_object2,
    resolution = as.numeric(chosen_resolution),
    verbose = F
  )
  ...
  if (remove_ig_genes) {
    ...
    seurat_object2[["Iggenes"]] <- Seurat::CreateAssayObject(
  ...
  plot_list[["Vlnplots"]] <- suppressWarnings(Seurat::VlnPlot(
  ...
  keep_clusters <- readline(
    'What clusters do you want to keep? Numbers separated only by commas: '
  )
  ...
  run_SCT <- as.character(readline(
    "Do you want to run SCTransform as well? Enter 'y' or 'n': "
  ))

  if (run_SCT == "y") {
    message("Running SCTransform")
    seurat_object2 <- Seurat::SCTransform(
      object = seurat_object2,
      verbose = FALSE,
      ncells = NULL
    )
...
enter which specific PCs you want to use, separated ONLY by commas: "
      ...
      if (grepl(pattern = ",", x = SCT_clustering_dims)) {
        ...
        seurat_object2 <- Seurat::FindNeighbors(
          object = seurat_object2,
          dims = 1:as.numeric(SCT_clustering_dims),
          verbose = TRUE,
          nn.method = nn.method
        )
        message("Calculating shiny new UMAP coordinates")
